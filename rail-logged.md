...
Not a dump of model outputs. **Logged** means an inspectable, trustworthy record of *what happened, who or what did it, on whose authority, why, and what resulted* — detailed enough to **prove the oversight catches mistakes, not merely that a human was nominally present** [D-14].

A confirmation click is not oversight if you can't later answer: *which* identity approved *what* action, on *whose* behalf, based on *what* the agent showed them, and whether the outcome was correct. "Logged" is what turns "we have a human in the loop" from a claim into something you can verify [C-20, D-13].

## Why a human in the loop is worthless without a real record

Four things become impossible without it:

- **You can't validate it.** The only honest test of oversight is whether humans actually *catch and correct* errors — the intervention-success rate. Gates reduce bad actions but barely improve catching them [A-17]; you only find out which is happening by measuring it, and you can only measure it from the record.
...
- **Intervention-success rate** — when the agent was wrong, how often did the human actually catch *and fix* it? (Not approval rate — correctness of approvals.) This is the number [A-17] shows is usually poor, and you can only get it from the log.
...
Without logs you can only *assert* oversight exists; with them you can *demonstrate* it catches mistakes — exactly the evidence good governance demands [D-14].

## Privacy, access control, and retention

The log is itself a sensitive, high-value asset — treat it like one:

- **Redact secrets and PII** at write time; never log credentials; consider field-level redaction for sensitive inputs/outputs.
...
- [ ] **L3** — **sub-agent provenance**: delegation chains, propagated principal + trace, a PROV-style graph, distributed tracing across agents.
...
*LoopRails · RAIL series · L — Logged. Companion to [`framework.md`](framework.md) and [`codex.md`](codex.md).*
