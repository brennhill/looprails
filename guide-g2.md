...
× stakes** — and the *highest* axis sets the grade. So a `git push` is G2 because it touches shared
state and is hard to reverse; sending an internal Slack message is G2 because the blast radius is
your team; spending $40 inside a $200 budget is G2 because real money moves. None of these is
catastrophic (that would be G3 — irreversible *and* external-or-severe, like deploying to prod or
paying a vendor), but none is free either.

The default LoopRails mode for G2 is **L3–L4**: confirm-before with a real preview, or
plan-approve for multi-step work. But "default" is not "answer." **The right design for a G2 action
depends entirely on whether a human can actually catch the mistake in time.** That is the whole
point of this page.

> One caveat before anything else: grade by *real* reversibility, not by whether your tool has an
> "undo" button. Checkpoint/undo usually covers an agent's structured edits but **not** shell side
> effects — a Bash command that deletes files is G2–G3 even if your editor can rewind. This is the
> shell-boundary trap `[A-4]`.

## Is a human in the loop the answer here? It depends — and that's the whole point.
...
- **Consequence** — how much oversight is *warranted* (reversibility × blast radius × stakes). For
  a G2 action this is, by definition, meaningful.
- **Controllability** — how much oversight can actually be *effective*. Can a person **detect** the
  problem from what's surfaced and **correct** it **in time**?

For a G2 action the consequence axis is fixed at "high enough to care." The *controllability* axis
is the swing variable, and it decides everything:

**If controllability is HIGH — a human can realistically catch the mistake in time — then a human
gate genuinely works. Invest in it.** A `git push` where the developer reads a clear, focused diff,
sees the failing test, and rejects: that is real oversight. Detectability is high (a diff is
verifiable), and there is time to act before harm lands. Build the gate, and build it well.

**If controllability is LOW — the human *cannot* reliably detect or correct in time — then a review
...
- **The recognition bottleneck `[A-17]`.** In a within-subjects study (n=48) of oversight
  ...
  — people talked themselves into the agent's answer. This is the **exposure-vs-correction gap**.
- **Automation bias.** Even motivated experts default to shallow review under productivity pressure
  ...
  heuristics, not systematic verification). A human told to "review the output" rubber-stamps it.
...
out-of-the-loop performance problem `[E-13]`. If there is no time to intervene, the gate is theater.

So the rule for G2 is: **where review works, design the review (next section). Where it doesn't,
prevent — don't review (the section after).**
...
actually succeeds (`framework.md` §5). Putting a human in the loop does *not* make them a good
...
4. **Give the provenance — "how did this even get to me?"** The upstream trail that restores
...
5. **Build detection affordances — support *checking*, not *trusting*.** Contrastive **why / why-not**
   explanations `[Q-16]` and diffs framed for verification; surface disagreement, counter-evidence,
   and the agent's own doubts.
6. **Add proportionate friction — a microboundary, not sludge** — *only* before the consequential
...
- **Preview / diff / dry-run.** Show the concrete effect before committing. This is the workhorse
  for G2 *only when the human can actually read the result*. A diff nobody can evaluate is decorative.
- **Plan-then-go (L4).** Approve a plan *before* execution, with checkpoints between steps. This is
  what expert developers actually do — upfront planning plus staged approval checkpoints `[A-20]` —
  and it is essential when the model can't be steered reliably mid-task.
- **Maker-checker (proposer ≠ approver).** For the upper edge of G2, borrow the four-eyes principle
  `[O-1]` and the two-person concept `[N-15]`: a second, *independent* party approves. Independence
  is the whole value — an agent re-reading its own work just confirms its own bias.

## How to prevent when review won't work

If controllability is low — the human can't tell right from wrong from what's shown, or there's no
time to intervene — adding a confirmation gate produces a rubber stamp, not safety. The valid moves
are prevention strategies that **don't depend on the human being a good detector** (`framework.md`
§1):

- **Shrink the consequence — make it reversible.** Undo beats confirmation `[G-14]`. If you can make
  the action one-click-reversible (push to a branch instead of `main`; a refund that can be clawed
  back), it drops a grade and the whole problem dissolves. Reversibility is a first-class safety
...
- **Sandbox.** Move the safety boundary off the per-action prompt and into the environment:
  no-network containers, scoped/ephemeral credentials, budget caps `[A-23]`. The worst case is then
  *contained*, not *reviewed* — the highest-leverage and most under-used control.
...
**increase acceptance regardless of correctness** `[F-20]` — they deepen over-reliance. Show
verifiable evidence (test results, the actual diff, the actual recipient list), not a confident
rationale.
...
**Is review the answer?** Usually yes — controllability is high. A diff is verifiable, tests give
...
- **Episode:** show the diff and exactly which branch/remote it affects; state whether it's
  reversible (a force-push to a shared branch is *not* — that re-grades toward G3); surface test
...
- **Prevent where review weakens:** if the diff is enormous, the human will rubber-stamp it — the
  recognition bottleneck bites `[A-17]`. So shrink the consequence: push to a feature branch behind a
...
- **Explanations that persuade instead of helping verification.** A polished rationale that sells
  ...
  explanation to help the human *find the error* — contrastive why/why-not `[Q-16]`, surfaced
  uncertainty `[Q-20]`, the actual diff — not to win the argument.
...
- **Friction as sludge.** Indiscriminate confirmations breed approval fatigue and habituation —
