...
The honest version of the core LoopRails question is uncomfortable: **can a human realistically catch this mistake in time?** For a fast, autonomous agent, the answer is usually no — and that is precisely why an automatic threshold beats an attentive human.
...
- **[G3 (critical)](guide-g3.html):** a circuit breaker is **required**, alongside prevention (sandboxing, capability locks, blast-radius caps) and a tested kill switch. At G3, review alone is a trap; the breaker is your containment when a human cannot catch the mistake in time.
...
- [ ] **The breaker auto-trips** when a threshold crosses — no human reaction time required.
