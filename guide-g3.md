...
dimensions — reversibility, blast radius, and stakes — and a single action that is irreversible *and*
...
usually a rubber stamp and a scapegoat, not a safeguard. Everything below explains why, and what to
build instead.
...
is nominally responsible and actually powerless. That is theater, and worse than theater: it
manufactures the *appearance* of control while removing the pressure to build the real thing.

So for a G3 action, treat "there is a human in the loop" as the *last* layer, not the first — and
...
**1. Contain it — move the safety boundary off the per-action prompt.** Run high-autonomy work in a
...
*time to intervene before harm lands*. The Knight Capital deploy cost ~$440M in 45 minutes because a
...
**6. Escalate or forbid when a human can't be a real safeguard.** If controllability is low — the
...
> **The mature G3 stack:** sandbox + scoped/expiring credentials + blast-radius caps → make it
> reversible (staging / soft-delete / delay) → verified runtime shield → maker-checker (proposer ≠
> approver) → break-glass for emergencies → escalate/forbid when the human can't really catch it.
...
outright [O-18]. *If a real purge is needed:* maker-checker — the agent proposes, an independent human
...
maker-checker with proposer ≠ approver [O-1]. The confused-deputy risk is acute — an injected
...
*killer items* (feature flags verified, dead code checked, rollback tested) — concise, because bloated
