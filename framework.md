...
> The codex asks *what is known*. This document asks *what to do about it*.
>
> **The four moves** map onto the sections below. **Grade** the action (§2). **Guard** it
> (§3 mode plus §4 control surfaces). **Show** the human (§5 loop episode). **Prove** it works
> (§6 defend plus §7 validate). The reframe (§0) and core model (§1) come first.
>
...
1. **Humans are unreliable detectors of automation error.** Automation bias and complacency are
   ...
   reduces bad *actions* but barely improves the human's ability to *catch* one. That is the
...
rubber-stamping [A-17]. The hard part is *allocation* of a scarce, leaky resource.

### The four A's of meaningful control

Test every oversight point against four conditions (synthesized from tracking+tracing
...
Most real systems get the interaction of the two axes wrong, and that interaction is where the
...
Everything in §2 through §7 is machinery for moving actions out of the top-left quadrant.
...
> files is G2 or G3 even if your editor has undo.
...
**The stage dial** picks *which* part of the agent's processing is overseen. Automate the early
stages freely. Gate the late ones, where over-trust and lost authority bite hardest [E-6]:

> acquire information → analyze → **recommend/decide** → **act**

**The level dial** sets *how much* the human is in control at the gated stage:
...
|---|---|---|
| **G0** | L0 to L1 | Spending attention here is waste; it manufactures alert fatigue [H-21, F-17] |
| **G1** | L1 to L2 | Reversibility plus notification is sufficient; undo beats confirmation [G-14] |
| **G2** | L3 to L4 | Block for approval; show a preview/diff; plan-approve multi-step work |
| **G3** | L4 to L5 **plus prevention**, and **L6 if controllability is low** | Review alone is a trap at this grade [A-17]; see §1 top-left quadrant |

That is the default. The **controllability overlay** then adjusts it.
...
- **High reviewer load / fan-out** (one human, many agents [H-22]): you cannot put every action at
  L3; reserve human attention for G2 and G3 and pool oversight.
...
- **trifecta exposure**: the action newly combines private data plus untrusted input plus external comms
  [A-23] (see §6);
...
The military/management answer to delegation transfers directly. Use **commander's intent plus
mission orders**: state the purpose, desired end-state, and hard constraints; specify *results to
attain, not how* [K-7]. Run **management by exception**: handle the routine autonomously,
...
5. **Sandbox / environment control**: no-network containers, scoped/ephemeral credentials, budget
   ...
   is the highest-value control by a wide margin, and it is underused.
...
Jidoka's stop-on-anomaly and the **andon cord** [N-1, N-4] are escalation plus a universal, blameless
interrupt. Finance's **circuit breakers and kill switches** [O-6, O-7] are automatic-halt plus
...
Steps §2 through §4 decide *which* actions get a human and at *what mode*. This step designs the
**moment itself**, which is where most real oversight quietly fails. The recognition bottleneck [A-17]
...
0. **Decide whether to interrupt at all (attention budget).** ⚠️*anti-fatigue*. An interruption is
   ...
   clinicians override 49% to 96% of alerts [H-10].
...
2. **Show the consequences *before* the human acts (feedforward).** ⚠️*clarity-of-consequence (your
   ...
   downstream side effects, and above all **whether it is reversible** [Q-3, Q-4]. Agents
   demonstrably propose irreversible, harmful actions ([Q-5]: even the safest agent fails ~24% on a
   high-stakes tool benchmark), so consequence plus reversibility preview is necessary, not decorative.
...
5. **Build in error-detection affordances, support *checking* not *trusting*.** ⚠️*detection*.
   ...
   disagreement, counter-evidence, and the agent's own doubts. Watch the trap: explanations framed
...
| **Alert fatigue**: too many low-value prompts and users dismiss all, including valid ones | [H-10, F-17, H-21] | **Budget interruptions**: every blocking prompt must demand a *specific* response [H-21]; cap the interruptive rate; most signals passive/logged; minimize false alarms (they corrode trust globally [F-17]); never fix fatigue by muting the noisiest category [H-11] |
| **Complacency from reliability**: better agent leads to less monitoring | [F-4, E-18, E-7] | Treat capability and complacency as rising together; keep humans actively engaged at decision points, not passive monitors [E-13]; vary/expose the agent's failure cases |
...
| **Moral crumple zone**: human positioned to absorb blame without control | [F-19, D-16, D-15] | Apply the four A's (§0); if you can't give real authority plus awareness plus ability, *don't claim oversight*, change the design |
...
- **Intervention success rate**: when the agent is wrong, how often does the human actually catch
  *and correct* it? (Not approval rate, but *correctness* of approvals.) This is the metric [A-17]
  shows is usually terrible; if you measure nothing else, measure this.
- **Override rate plus override correctness**: are humans overriding, and are their overrides right?
...
2. GRADE    Score each by reversibility × blast radius × stakes → G0 to G3.         (§2)
...
| **2, Reversible & sandboxed** | Undo-by-default plus environment containment; gates reserved for the irreversible. [A-4, A-23] |
...
- **Map/Grade:** read file (G0) · edit local file (G1, reversible via checkpoint) · run tests (G1)
  ...
  severe).
- **Match:** reads/edits autonomous+logged with undo (L1 to L2); `git push` confirm-before with diff
...
- **Defend:** combat rubber-stamping of large diffs with focused, reviewable chunks plus tests as
...
- **Match:** answer autonomously with deterministic guardrails it "cannot cross" (e.g., a 30-day
  window) [A-15]; in-policy refunds L1 to L2; out-of-policy escalate to a human with an AI-generated
  summary [A-15, A-6].
...
human-factors / HCI / AI-safety base (Parts I and II), it now draws on:
...
Parts III to V and the loop-episode section after the initial draft.*
