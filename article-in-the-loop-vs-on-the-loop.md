...
The agent proposes; the human disposes. Each consequential action waits for explicit approval before it executes. The human is *inside* the control loop, and the system literally cannot advance without them. This is the strongest form of control and the most expensive: it adds latency to every action and demands the human's attention every time.
...
The pattern is a tension, not a free lunch. More autonomy buys throughput and frees human attention, and it simultaneously erodes the human's ability to take over when takeover finally matters. Push autonomy all the way up with only a passive monitor, and you have maximized both your speed *and* your takeover penalty.
...
The right mode is a function of two things: how bad the outcome could be (consequence) and how realistically a human could steer or stop it in time (controllability). LoopRails grades each action **G0 (trivial) to G3 (critical)** along reversibility, blast radius, and stakes, and matches controls to the grade.

- **Low consequence (G0 to G1):** Let the agent run. On-the-loop or fully automated is fine; a per-action approval here is pure friction that trains the human to click "yes" reflexively, which then poisons their judgment on the prompts that matter.
...
That last case is the crux. The core LoopRails question is not "should a human review this?" It is: **"Can a human realistically catch this mistake in time?"** If the honest answer is no, an approval prompt is just a delay with a signature on it, not oversight.
...
This is also why "more approval prompts" is not the answer to a risky agent. Research on AI coding agents (see the [LoopRails codex](./codex.html)) found that plan-approval reduced attack occurrence from roughly 90% to 60 to 74%, a real but partial dent, while human intervention success stayed at only 9 to 26%. Humans were not reliably catching the bad actions even when shown a plan. That is the rubber-stamp problem in numbers: putting a person in the loop does not help much if they cannot actually catch the mistake. Prevention beats review when review would not work.
...
Stop guessing which mode an action needs. Grade it. Run your real actions through the [LoopRails grader](./index.html#grader) to get a G0 to G3 grade and matched controls, read the full method in the [framework](./framework.html), and use the [playbook](./playbook.html) and [cheatsheet](./cheatsheet.html) to put it into practice today. Keep your agents on the rails, and keep the human in the one loop where they can actually help.
