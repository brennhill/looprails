...
If the answer is no, then a review step is not a safety control. It is theater. The agent still does the wrong thing; you have simply added a person whose name is on the approval. The [framework](framework.html) reframes oversight around this distinction, and it changes nearly every design decision that follows.

## The evidence: an approval click is not the same as catching an error

Here is the finding that should reset everyone's intuition. In research on AI coding agents (see the LoopRails [codex](codex.html)), requiring plan-approval did reduce how often attacks occurred, from roughly 90% down to 60-74%. That sounds like a win. But the number that actually matters for safety stayed grim: when a bad action was put in front of a human to catch, intervention success was only 9-26% across every oversight strategy tested.
...
Why? Two well-documented forces.
...
**The rubber stamp.** A human told to "review the output" under any time pressure will skim and click approve. The agent's proposal arrives wrapped in a confident rationale; the reviewer reads the rationale, it sounds reasonable, and they accept it. This is the **Rubber Stamp** anti-pattern, and it is the default outcome of naive oversight, not the exception.
...
Now the dangerous quadrant: consequence is high but controllability is low. The human *cannot* reliably detect or correct the error from what's surfaced, or there isn't time. **Review is a trap.**

Putting an approval gate here does not produce safety. It produces a rubber stamp and a scapegoat. The recognition bottleneck and automation bias guarantee the human accepts; the 9-26% figure is exactly this quadrant in the data. You have manufactured the *appearance* of control over an action no human in that position could actually control.

And it gets worse than ineffective, because it creates a **moral crumple zone**: a human positioned to absorb blame for a system's failure despite having no real power to prevent it. The reviewer's signature is on the approval, so when the agent deletes the production database or wires the payment, accountability collapses onto them. The system and its designers are insulated. The human is the liability sponge. That is not a safety mechanism; it is a way of laundering responsibility for a design that was never safe.
...
When review is a trap, the answer is not a better prompt. It is to stop depending on the human as a detector and prevent the bad outcome directly. The [playbook](playbook.html) is built around the method **Grade, Guard, Show, Prove**.
...
- **Kill Switch** so there is always a way to stop. Knight Capital lost about $440M in roughly 45 minutes in 2012 to trading software with no way to halt it. A missing kill switch is not an edge case; it is how the worst incidents happen.
...
- **An approval click is not error-catching.** Plan-approval cut bad actions from ~90% to 60-74%, but human intervention success stayed only 9-26% (see the [codex](codex.html)).
