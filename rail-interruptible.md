...
cheaply, and without having to justify themselves first. "Anyone" is literal here. A teammate
watching over a shoulder, an automated monitor that trips on an anomaly, or the end user who
realizes mid-run "that's not what I meant." Each of them needs a low-friction, blameless authority
to halt the agent before it does the next thing.
...
1. **A blameless, universal stop.** Stopping costs nothing socially. Nobody has to be *sure* the
   ...
   to stop the production line the instant they see a defect. Workers are *rewarded* for pulling it,
...
2. **One kill switch that halts everything at once.** Beyond the per-action pause, there is a single
   ...
   cancel working orders with no root-cause analysis required. Stop first, investigate later [O-7].
...
People assume "you can always stop it" is self-evidently true. There's a Ctrl-C, there's a stop
button, the agent is *told* to obey interruptions. That confidence is misplaced, and the reason is
empirical.

**Today's models are weak at honoring mid-task interruption, cancellation, and re-planning.** This
is a model capability gap, not a UI gap. The InterruptBench benchmark tested six frontier LLMs on
long-horizon web tasks across three interruption types: *adding* a goal, *revising* the current
goal, and *retracting* it entirely. Even the best models struggled to adapt and recover when the
user changed their mind mid-flight [A-19]. Field evidence agrees. Coding agents degrade sharply on
iterative problem-solving and mid-task scope changes [A-16], and the dominant real-world failure
mode is the agent continuing on its own trajectory while the developer pushes back [A-18].

So a "steer it mid-run" affordance that *routes the new instruction back into the same model* is not
a reliable stop. The model may acknowledge the interruption and then keep doing roughly what it was
doing. Interruptibility has to be **engineered around the model, not delegated to it.** The
guaranteed stops live *outside* the agent's reasoning, in the orchestrator, the process supervisor,
the credential broker, the network. The model's cooperation helps on the happy path but is not the
safety mechanism. Treat steerability as a capability you must *test* per model, and never build a
...
Interruptibility is a layered stack, running from polite to brutal. Build all the layers, and rely
on the lower, model-independent ones for anything consequential.
...
- **Cancellation tokens.** Every step checks a shared token before it acts. On cancel, the loop exits
  ...
  this pattern: a run pauses, surfaces an interruption, and resumes from saved state once resolved
  [A-7, A-8].
- **Checkpointing between steps.** Snapshot durable state at each step boundary so a stop never lands
  ...
  built on a checkpointer, since HITL *requires* durable persistence underneath [A-8].
...
- **Revoke the credentials.** Invalidate the scoped, short-lived tokens the agent acts through so its
  *next* tool call fails even if the process is still alive. This is lockout/tagout from industrial
  safety: positively de-energize the system's ability to act rather than merely asking it to stop
  [N-17].
...
- **Cancels working actions, not just new ones.** Blocking new tool calls while letting in-flight
  ...
  resting.
- **Both manual and automatic.** A human can pull it, and a limit breach can also pull it (see
  circuit breakers).
...
[O-4]. The absence of a single decisive stop turned a small config error into a company-ending event.
...
The breaker stops *before* a human notices, and the mandatory re-authorization makes resuming a
deliberate human re-entry point rather than a silent restart [O-6]. This mirrors the factory
*jidoka* primitive: full speed on the happy path, automatic hard stop on anomaly, escalate to a
human [N-4].
...
| **Use for** | routine pause/redirect, low-to-medium grade | high grade, runaway, injection, panic |

Offer graceful as the default and keep hard always armed underneath. Never let "graceful" be the
*only* option for a consequential action. That re-introduces the model dependency you were trying to
escape.
...
- **Early and gradual.** Hand back *before* the cliff, not at it, and step authority down in stages.
  A firehose hurts most during a crisis, so prioritize and interpret *for* the human instead of
  dumping everything [H-7].
- **Context-rich.** Carry the provenance: what the agent saw, what it tried, and *why* it stopped or
  escalated. That is situation awareness, not a log dump [framework §5; E-13].
- **Budget the re-engagement latency.** Takeover is not instantaneous. Driving-automation studies put
  human takeover at ~1.5 to 3.5 seconds, longer under secondary-task load, and longer still to
  rebuild the *competence* to act well [H-19]. For time-critical failures, the human fallback may
  simply not arrive in time, so don't architect safety around a handoff that can't re-engage fast
  enough [H-19].
...
- **No kill switch (the Knight Capital runaway).** Trusting that "we can always pull the plug" without
  building the plug. Knight had a fast autonomous actor and no decisive stop, and ~4M bad orders and
...
- **Gameable "are you still there?" engagement checks.** Token engagement nags, a hands-on-wheel ping
  or a periodic "click to continue," feel like oversight but verify nothing. Tesla's gore-point
  fatality involved a driver playing a game past a non-effective engagement-monitoring design [H-18],
  and the Uber/Tempe fatality involved a safety driver on her phone behind a system reliable enough
  to induce disengagement but not to trust [framework §6]. An interruption check that a disengaged
  human can satisfy without re-engaging is theater. Verify *meaningful* engagement, or budget for the
  fact that the human is out of the loop.

- **Assuming the model will just stop when told.** The most seductive mistake is wiring "interrupt" to
  a message back into the same model and calling it done. The benchmark says frontier models are weak
  at honoring mid-task changes [A-19], the field data says agents keep going until the human forcibly
  pushes back [A-18], and a polite cancel is worthless under prompt injection. Build the guaranteed
  stop outside the model, and let the model's cooperation be a bonus rather than the mechanism.
...
*LoopRails · RAIL series · I is for Interruptible. See also: R (Reversible), A (Authorized), L (Logged).*
