...
act, tell the human what it did, and make the undo cheap. Reversibility is the safety net, not
confirmation. A blocking approval gate here buys you almost nothing and trains the human to
...
> edits but **not** shell side-effects. A Bash `rm`, `mv`, or `cp` is G2 to G3 even if your editor
> has undo, because the destructive operation sits outside the recovery net [A-4]. "Edit a local
> file" is G1; "let the agent run arbitrary Bash that touches files" is not.
...
exactly this kind of barrage [H-10]. False-alarm-prone signals are corrosive in a way teams
underestimate. They damage *both* compliance and reliance, so a human trained to click through your
dependency-install prompts will also click through the `git push` prompt that mattered [F-17].
Every G1 gate you add spends down the attention budget you need for G2/G3.
...
**Autonomy level: L1 to L2 (act, then notify).** LoopRails separates two dials: *which* stage of
the agent's processing is gated (acquire → analyze → decide → **act**) and *how much* the human is
in control at that stage [E-6]. For G1 the default mapping is **L1 to L2** on the *act* stage
(framework §3):
...
G2 to G3 no matter how good your rewind button is. So:
...
- **What changed + that it's reversible:** the concrete effect and a one-click undo affordance
  (consequence plus reversibility, the feedforward principle, here as feedback) [Q-3, Q-4].
...
**Keep interruptions within budget.** The whole point of L1 to L2 is to *not* spend attention here.
...
- **Grade.** Reading files is G0; **editing a local file is G1** (reversible via checkpoint, local
  blast radius); **running tests is G1**. (`git push` would be G2, since it's shared and hard to
  reverse, and `rm -rf` G3; those get gates and escalation, not this treatment.)
- **Match.** Edits and test runs are **autonomous + logged with undo (L1 to L2)**. The agent does
  not ask before each edit. It *does* auto-escalate if an edit reaches outside the approved scope
  (e.g., files the task never mentioned) [A-18]: dynamic, not static, autonomy.
...
local action. It feels responsible and does the opposite. Gates barely improve catching [A-17], and
the habituation they breed gets you tuned out by the second prompt [O-16], spending the attention
budget you need for real decisions. **Antidote:** Checkpoint & Rewind, act, notify, undo [G-14, A-4].
...
- **[A-17]** Empirical HITL study: the **recognition bottleneck**, gates cut bad actions (~90% to
  60-74%) but intervention success stays 9-26%.
