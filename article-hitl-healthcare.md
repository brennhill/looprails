...
In a clinical setting, the high-stakes actions are exactly the ones a human cannot reliably catch after the fact. An auto-sent order or patient message acts on a real person before anyone reviews it. So the control is not "let the AI act and let a clinician check it later." That is not oversight; it is hoping the model was right.

The control is to keep a human in command. The AI recommends and a licensed clinician decides and executes any action that touches a patient. Decision support, not autonomous action. When you cannot safely catch the mistake in time, you do not automate the action — you keep it a recommendation a human owns. The [playbook](playbook.html) and [cheatsheet](cheatsheet.html) walk through wiring this in.
...
- Match controls to grade: recommend-don't-act for high stakes, alerts tuned hard against fatigue, evidence and sources shown not just a verdict, accountability matched to real control, and a full audit log.
