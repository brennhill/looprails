...
LoopRails frames this with a single test: *can a human realistically catch this mistake in time?* When the answer is no, the correct move is to **prevent the bad outcome, not gate it.** That is the line between security theater and a real boundary. See the [interruptibility RAIL](rail-interruptible.html) for when a stop is genuinely possible, and when it is not.
